...
class Window(Frame):
    ...
    def mode_zip(self):
        ...
        name = askopenfilename(initialdir="" + self.def_path, filetypes=(("Zip File", "*.zip"), ("All Files", "*.*")), title="Choose Zip Plugin")
    ...
    def test_ip(self):
        ...
        if os.name == 'nt':
            ...
            if ('unreachable' in rep):
                ...
                self.lbl.configure(text="No Connection... !!", background= "red")
            else: 
                print('server is up')
                self.lbl.configure(text="Connection OK... !!", background= "green")
        else :
    ...
    def save_log(self):
        ...
        f = asksaveasfile(mode='wb' , filetypes=(("Log File", "*.log"), ("All Files", "*.*")), title="Save Log File" )
    ...
    def mode_url(self):
        ...
        if len(ext_url_movian) >= 7 :
            bnt2.configure(text = "Send Link", command=mode_url)
            webbrowser.open(f'http://{ip_movian}:{self.port_movian}/showtime/open?url={ext_url_base_name}', new=1, autoraise=True)
            self.lbl.configure(text=f'{ext_url_base_name}')
        else:

            self.lbl.configure(text="Put the correct URL or use Zip mode... !!")

   

    def about(self):
        messagebox.showinfo('About', 'Movian One Click Plugin Installer by nikkpap\n with the great help of Czz78 (thanks man)\n <<for MovianGR and Movian on Telegram>>\n ALU DEV TEAM @ 2019-20 ')
...
if __name__ == "__main__":
    ...
    if os.name == 'nt':
        ...
        ip_address = get_ip_address().split()[0] # this function on windows returns other ip, this is in array because in linux you can have more ip in sequence, i don't remember on win
